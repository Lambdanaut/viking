...
public:
	...
	// in debug builds, will return the original hashed string.
	// in debug builds, causes undefined behaviour if the string originally used to construct the hash no longer exists. 
	// in release builds, will return an empty string.
	inline const char* getDebugString() const;
...
const char* HashedString::getDebugString() const
{
#ifdef _DEBUG
